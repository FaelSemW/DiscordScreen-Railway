...
describe('resumoPeer', () => {
  ...
  it('cobre ramificações de encodings vazios, áudio e candidatos não-nominados', async () => {
    // 1. Criar peer com ICE default (null)
    const pcDefault = criarPeer({ ice: null });
    expect(pcDefault.config.iceServers).toBeDefined();

    // 2. Ajustar envio com áudio e encodings vazio
    const audioSender = sender('audio', { encodings: [] });
    const pcAudio = criarPeer({ ice: [] });
    pcAudio.senders = [audioSender];
    await ajustarEnvio(pcAudio, { bitrate: 64000 });
    expect(audioSender.aplicados.length).toBe(1);

    // 3. Resumo peer com candidato sem currentRoundTripTime e com nominated: false
    const pcPairs = comEstatisticas([
      { id: 'p1', type: 'candidate-pair', state: 'succeeded', nominated: false },
      {
        id: 'p2',
        type: 'candidate-pair',
        state: 'succeeded',
    ...
    const resumo = await resumoPeer(pcPairs);
    expect(resumo.rtt).toBeNull();
  });
});
