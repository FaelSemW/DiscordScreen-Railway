...
import {
  ...
  supportError,
  QUALITY_PRESETS,
  ADAPTIVE_LADDER_FLUIDEZ,
  ADAPTIVE_LADDER_QUALIDADE,
} from './broadcaster.js';
...
describe('conexão', () => {
  ...
  it('encerra quando a conexão cai sozinha', async () => {
    const onEnd = vi.fn();
    const { ws } = await noAr({ onEnd, maxReconnectAttempts: 0 });
...
describe('setQuality', () => {
  it('reconfigura o encoder e pede a taxa nova à captura', async () => {
    ...
    expect(b.getSettings()).toMatchObject({ bitrate: 5_000_000, fps: 60 });
  });
  ...
  it('mantém o que não foi pedido', async () => {
    ...
    expect(b.getSettings()).toMatchObject({ bitrate: 2_500_000, fps: 30 });
  });

  it('aplica preset economia com 720p30 e 2Mbps', async () => {
    const { b, encoder } = await noAr();

    b.setQuality({ preset: 'economia' });

    expect(encoder.configuracoes.at(-1)).toMatchObject({ bitrate: 2_000_000, framerate: 30 });
    expect(b.getSettings()).toMatchObject({ preset: 'economia', bitrate: 2_000_000, fps: 30 });
  });

  it('aplica preset maxima com 1080p60 e 8Mbps', async () => {
    const { b, encoder } = await noAr();

    b.setQuality({ preset: 'maxima' });

    expect(encoder.configuracoes.at(-1)).toMatchObject({ bitrate: 8_000_000, framerate: 60 });
    expect(b.getSettings()).toMatchObject({ preset: 'maxima', bitrate: 8_000_000, fps: 60 });
  });
});

describe('presets de qualidade e adaptação', () => {
  it('exporta presets definidos com os parâmetros corretos', () => {
    expect(QUALITY_PRESETS.economia).toMatchObject({ fps: 30, bitrate: 2_000_000 });
    expect(QUALITY_PRESETS.equilibrado).toMatchObject({ fps: 30, bitrate: 4_000_000 });
    expect(QUALITY_PRESETS.alta).toMatchObject({ fps: 30, bitrate: 6_000_000 });
    expect(QUALITY_PRESETS.maxima).toMatchObject({ fps: 60, bitrate: 8_000_000 });
    expect(QUALITY_PRESETS.automatico).toMatchObject({ isAuto: true });
  });

  it('inicia no nível mais alto da escada em modo automático', async () => {
    const { b, encoder } = await noAr({ preset: 'automatico', priority: 'fluidez' });
  ...
  it('suporta escada com prioridade em qualidade', async () => {
    const { b, encoder } = await noAr({ preset: 'automatico', priority: 'qualidade' });

    expect(encoder.configuracoes.at(-1)).toMatchObject({
      bitrate: ADAPTIVE_LADDER_QUALIDADE[0].bitrate,
      framerate: ADAPTIVE_LADDER_QUALIDADE[0].fps,
    });
    expect(b.getSettings()).toMatchObject({
      preset: 'automatico',
      priority: 'qualidade',
  ...
  it('adapta escada para baixo após 5 segundos contínuos de sobrecarga', async () => {
    vi.useFakeTimers();
    try {
      const onStats = vi.fn();
      prepararCaptura(telaSimples());
      const b = createBroadcaster(opcoes({ preset: 'automatico', priority: 'fluidez', onStats }));
      const promessa = b.start();
      ...
      // Simula 5 segundos consecutivos de encoder sobrecarregado (fila alta)
      for (let i = 0; i < 5; i++) {
        encoder.encodeQueueSize = 5;
        await vi.advanceTimersByTimeAsync(1000);
      }

      expect(b.getSettings().adaptiveLevel).toBe(1);
      expect(encoder.configuracoes.at(-1)).toMatchObject({
        bitrate: ADAPTIVE_LADDER_FLUIDEZ[1].bitrate,
        framerate: ADAPTIVE_LADDER_FLUIDEZ[1].fps,
      });
    } finally {
      vi.useRealTimers();
    }
...
describe('estatísticas', () => {
  it('reportam telemetria detalhada de taxa configurada, codificada, enviada e frame pacing', async () => {
    vi.useFakeTimers();
    try {
      const onStats = vi.fn();
      prepararCaptura(telaSimples());
      const b = createBroadcaster(opcoes({ preset: 'automatico', onStats }));
      const promessa = b.start();
      await vi.advanceTimersByTimeAsync(0);
      sockets.at(-1).abrir();
      await promessa;
      ...
      expect(onStats).toHaveBeenCalledWith(
        expect.objectContaining({
          viewers: 7,
          preset: 'automatico',
          bitrate: expect.objectContaining({
            configuredBps: expect.any(Number),
            encodedBps: expect.any(Number),
            sentTotalBps: expect.any(Number),
          }),
          capture: expect.objectContaining({
            requestedFps: expect.any(Number),
            avgIntervalMs: expect.any(Number),
            p95IntervalMs: expect.any(Number),
          }),
          encoder: expect.objectContaining({
            queueSize: expect.any(Number),
            avgEncodeMs: expect.any(Number),
            outputGapAvgMs: expect.any(Number),
          }),
          health: expect.any(String),
          mbps: ((18 + CARGA.length) * 8) / 1e6,
        }),
      );
    } finally {
      vi.useRealTimers();
