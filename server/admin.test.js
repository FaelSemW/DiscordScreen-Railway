...
describe('buildAdminDashboard', () => {
  ...
  it('lida com servidores sem nome e pings vazios com segurança', () => {
    const painel = montar([sala('a', { guildId: 'g-anon', guildName: null, isCall: true })]);

    expect(painel.guilds[0].name).toBeNull();
    expect(painel.summary.pingAverageMs).toBeNull();
    expect(painel.summary.pingMedianMs).toBeNull();
    expect(painel.summary.pingP95Ms).toBeNull();
  });
});
